...
public class CharImageRender implements ImageRenderer{
    private final BufferedImage sourceImage;
    private final List<String> words;
    private final int gridSize;
    //smaller font size = more detailed image. fontSize should be <= gridSize
    private final Font baseFont = new Font("Times New Roman", Font.PLAIN, 12);

    public CharImageRender(BufferedImage src, List<String> text){
        sourceImage = src;
        words = text;
        gridSize = words.get(0).length();
    }

    public CharImageRender(BufferedImage src, List<String> text, int blockSize){
        sourceImage = src;
        words = text;
